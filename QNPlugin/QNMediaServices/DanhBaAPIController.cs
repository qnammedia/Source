...
namespace QNMedia.CMSAPI
{
    ...
    public class DanhBaAPIController : DnnApiController
    {
        CMSController ctrl = new CMSController();
        QNMediaHelper hp = new QNMediaHelper();
        [HttpPost()]
        public HttpResponseMessage Update([FromBody] CMS_DanhBaDienThoai info)
        {
            try
            {
                ...
                {
                    info.Img = hp.GetCMSImgURL(info.Img, 110, 120);
                }
                var vb = ctrl.CMS_DanhBaDienThoai_Update(info);
                if (vb != null)
                {
        ...
        public HttpResponseMessage Update_Public([FromBody] BaseUpdateInfo info)
        {
            ...
            {
                info.PortalID = PortalSettings.PortalId;
                ctrl.CMS_DanhBaDienThoai_UpdateSuDung(info);
        ...
        public HttpResponseMessage UpdateSortOrder([FromBody] BaseUpdateInfo info)
        {
            ...
            {
                info.PortalID = PortalSettings.PortalId;
                ctrl.CMS_DanhBaDienThoai_UpdateSortOrder(info);
                return Request.CreateResponse(HttpStatusCode.OK, new StatusInfo { status = "OK" });
            }
            catch (Exception ex)
            {
        ...
        public HttpResponseMessage Delete([FromBody] BaseUpdateInfo info)
        {
            ...
            {
                info.PortalID = PortalSettings.PortalId;
                ctrl.CMS_DanhBaDienThoai_Delete(info);
                return Request.CreateResponse(HttpStatusCode.OK, new StatusInfo { status = "OK" });
            }
            catch (Exception ex)
            {
        ...
        public HttpResponseMessage Get()
        {
            try
            {
                var lst = ctrl.CMS_DanhBaDienThoai_Get(PortalSettings.PortalId);
        ...
        public HttpResponseMessage GetByID(int ID)
        {
            try
            {
                var ob = ctrl.CMS_DanhBaDienThoai_Get(PortalSettings.PortalId).Where(x => x.ID == ID).FirstOrDefault();
        ...
        //    try
        //    {
        //        var ob = ctrl.CMS_DanhBaDienThoai_GetByModule(PortalSettings.PortalId, tmid);
        //        return Request.CreateResponse(HttpStatusCode.OK, new StatusInfo { status = "OK", result = ob });
        //    }
        //    catch (Exception ex)
        //    {
